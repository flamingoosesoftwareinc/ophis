...
// enableCommand creates a Cobra command for adding an MCP server to Claude Code.
...
func enableCommand(commandName string, defaultEnv map[string]string) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "enable",
		Short: "Add server to Claude Code config",
		Long:  "Add this application as an MCP server in Claude Code",
